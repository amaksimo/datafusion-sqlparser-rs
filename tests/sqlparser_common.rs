...
fn parse_create_sequence_clause_order_independent() {
    // CREATE SEQUENCE option clauses are order-independent standard PostgreSQL
    // syntax parseable by the generic dialect, so this is exercised across all
    // dialects via the common harness.
    //
    // pg_dump emits options in an order that a positional parser could not
    // handle (START before INCREMENT). It must parse cleanly.
    ...
    // The parser preserves clause order in the AST vector, so two different
    // orderings of the same logical sequence parse into *different* vectors,
    // each matching the literal order it was written. The point here is that
    // both orderings parse successfully into the order written.
